 ...
 *
 * Date: Thu Apr 17 2014 17:54:38 GMT+0200 (CEST)
 */
...
console.log("priv:"+JSON.stringify(priv));
  ...
  this.selection = selection = { //this public assignment is only temporary
    ...
    setRangeStart: function (coords) {
      if( coords.row > instance.countRows() ) {
          coords.row = instance.countRows()-1;
      }
      if( coords.col > instance.countCols() ){
          coords. col = instance.countCols();
      }
      if( coords.row > instance.countRows() || coords.col > instance.countCols()) {
        return false;
      }
    ...
    empty: function () {
      ...
      //check for formula and delete accordingly
      var isExistFormula = checkRangeData(topLeft, bottomRight, instance, "delete");
      var checkCellBelongToFormula = checkRangeData(topLeft, bottomRight, instance, "checkCellBelongToFormula");

	  if(isExistFormula){
		  new Modal({id : 1,
	    		headingText : "Are you sure?",
				containtText : "The field you're editing contains a formula. Are you sure you want to delete it? This cannot be undone.",
	    		 okActionEvent : function(){
	    			 var deleteStatus = deleteComponent(topLeft, bottomRight, instance, true);
	    			 if(deleteStatus){
	  ...
	  } else if(checkCellBelongToFormula){
        new Modal({id : 1,
          headingText : "Are you sure?",
          containtText : "The field you're deleteing belongs to a formula. Are you sure you want to delete it? This cannot be undone.",
           okActionEvent : function(){
        	 var deleteStatus = deleteComponent(topLeft, bottomRight, instance, true);
        	 if(deleteStatus){
                instance.setDataAtCell(changes);
    	  ...
    	  var deleteStatus = deleteComponent(topLeft, bottomRight, instance, false);
    	  if(deleteStatus){
		    instance.setDataAtCell(changes);
    	  }
	  }
      
      //deleteComponent(topLeft, bottomRight, instance);
      //instance.setDataAtCell(changes);
	  //showCautionIcon(topLeft, bottomRight, instance, "removeEntry");
    }
  };
  ...
  this.pyableData = function (startRow, startCol, endRow, endCol) {
  ...
  this.setCellReadOnly = function (row, col) {
    $(instance.view.getCellAtCoords(new WalkontableCellCoords(row, col))).data("readOnly", true);
  };
  ...
  this.setCellEditable = function (row, col) {
	  $(instance.view.getCellAtCoords(new WalkontableCellCoords(row, col))).data("readOnly", false);
   };
...
Handsontable.TableView = function (instance) {
  ...
  var walkontableConfig = {
      ...
      current: {
        ...
        border: {
          ...
          cornerVisible: function () {
        	...
        	selectedTableRange = instance.getSelectedRange();
        	selectedTableCells = instance.getSelected();
        	/*var info = instance.mergeCells.mergedCellInfoCollection.getInfo(sel[0], sel[1]);
            if (info) {
              //console.log('Unmerge cells');
              if(typeof itsId != "undefined" && itsId != null){
                  $("#SplitCellsBtnId_"+itsId).removeAttr("disabled");
                  $("#SplitCellsBtnId_"+itsId).removeClass("checkBoxclick");
                  $("#MergeCellsBtnId_"+itsId).attr({"disabled":true, "checked":false});
                  $("#MergeCellsBtnId_"+itsId).addClass("checkBoxclick");
               }
            }
            else {
              //console.log('Merge cells');
              if(typeof itsId != "undefined" && itsId != null){
                 $("#SplitCellsBtnId_"+itsId).attr({"disabled":true, "checked":false});
                 $("#SplitCellsBtnId_"+itsId).addClass("checkBoxclick");
                 $("#MergeCellsBtnId_"+itsId).removeAttr("disabled");
                 $("#MergeCellsBtnId_"+itsId).removeClass("checkBoxclick");
              }
            }*/
    ...
    onCellMouseDown: function (event, coords, TD) {
      if($(event.target).hasClass('tableOption')){
        return false;
      }
      instance.listen();
      ...
      items: {
        'row_above': {
          ...
          callback: function(key, selection){
        	/* reset previous row */ 
        	var cmptId = $(instance.rootElement).attr("id").split("_")[1];
        	var tdCollection = $(instance.getCell(selection.start.row, selection.start.col)).parent().find("td");
        	var cellIds = [];
        	var ind = 0;
        	
        	var nextRow = selection.start.row + 1;
        	for(ind; ind<tdCollection.length; ind++){
        		cellIds.push($(tdCollection[ind]).attr("id"));       		
        	}
            var row = instance.countRows() + 1;
            var col = instance.countCols();
          
            var cmpCollection = getCurrentCellIdsComponents(cmptId, cellIds); // holds current selected index row contents before addition
          
        	updateHtmlCollection(cmptId, cellIds, 'updateObject', row, col);
        	
            this.alter("insert_row", selection.start.row);
          
            /* shift cell values row wise */
          
            var trLength = $(instance.rootElement).find("tr").length - 1;
            var sCounter;
            var rowindx = selection.start.row; // holds current selected row index
            
            /* loop for shifting cell values in last-secondLast row fashion */
            for(sCounter=trLength-1; sCounter > rowindx ; sCounter--){
        		var newcellIds = [];
            	var newcellstoShift = [];
            	
            	var nextTrParent1 = $(instance.rootElement).find("tr").eq(sCounter);   //last row
            	var nextTrParent2 = $(instance.rootElement).find("tr").eq(sCounter+1); //second last row
            	var ind = 0;
            	    
            	for(ind; ind<tdCollection.length; ind++){              	  
            		var childTd1 = $(nextTrParent1).children('td').eq(ind);
            		if(childTd1.length > 0){
            			newcellIds.push($(childTd1).attr("id"));
            		}
            		
            		var childTd2 = $(nextTrParent2).children('td').eq(ind);
            		if(childTd2.length > 0){
            			var cell_id2 = childTd2.attr("id");
            			newcellstoShift.push({"id": cell_id2, "row":sCounter, "col":ind});
            		}
            		
            	}              	
            	//console.log("newcellIds "+newcellIds);
              	//console.log("newcellstoShift "+newcellstoShift);
              	
              	var newcmpCollection = [];
              	if((sCounter-1)==rowindx){
              		newcmpCollection = cmpCollection;  // for current selected index row contents before addition
              	}else{
              		newcmpCollection = getCurrentCellIdsComponents(cmptId, newcellIds);
              	}
              	if(newcmpCollection.length >0 && newcellstoShift.length > 0){
              	  shiftHtmlCollection(cmptId, newcellstoShift, newcmpCollection); // shifts collection
              	}            	
            }
          
            this.render();  // render table to show updates
            
            resetCellHtml(cmptId); // reset cell html on data change
          
            $("#SpreadsheetTable_"+cmptId).trigger("click");
          },
          disabled: function () {
          ...
          callback: function(key, selection){
            ...
            for(ind; ind<tdCollection.length; ind++){
             cellIds.push($(tdCollection[ind]).attr("id"));
              ...
              var col = instance.countCols();
              //updateHtmlCollection(cmptId, cellIds, 'updateObject', row, col);
              updateSpreadSheetData(cmptId,row, col);
              
              /* shift cell values row wise */
              var trLength = $(instance.rootElement).find("tr").length - 1;
              trLength = trLength - 1; // to reduce loop counter
              var sCounter;
              var rowindx = selection.start.row; // holds current selected row index
              
              /* loop for shifting cell values in secondLast-last row fashion */
              for(sCounter=trLength-1; sCounter > rowindx ; sCounter--){
            		var newcellIds = [];
                	var newcellstoShift = [];
                	
                	var nextTrParent1 = $(instance.rootElement).find("tr").eq(sCounter+1); //second last row
                	var nextTrParent2 = $(instance.rootElement).find("tr").eq(sCounter+2); //last row
                	var ind = 0;
                	...
                	for(ind; ind<tdCollection.length; ind++){              	  
                		...
                		if(childTd2.length > 0){
                			var cell_id2 = childTd2.attr("id");
                			newcellstoShift.push({"id": cell_id2, "row":sCounter+1, "col":ind});
                		}
                	...
                	if(newcellstoShift.length > 0 && newcmpCollection.length > 0 ){
                  	   shiftHtmlCollection(cmptId, newcellstoShift, newcmpCollection); // shifts collection
                  	}
                	
              }
              
              var tdCollection = $(instance.rootElement).find("tr").eq(rowindx+2).find("td");
              var nextRowCellIds = []; // hold ids of row cells next to current selected  
              var cnt = 0;
              for(cnt; cnt<tdCollection.length; cnt++){
            	  nextRowCellIds.push($(tdCollection[cnt]).attr("id"));
              }
              
              if(nextRowCellIds.length > 0){
            	  updateHtmlCollection(cmptId, nextRowCellIds);  // reset row cells next to current selected
              }
        ...
        'col_left': {
            ...
            callback: function(key, selection){
          	...
          	var cmptId = $(instance.rootElement).attr("id").split("_")[1];
          	var colindx =  selection.start.col;
          	var trCollection = $(instance.getCell(selection.start.row, selection.start.col)).parents("table").find("tr");
          	trCollection.splice(0,1) //remove first unwanted tr that has th collection
          	var cellIds = [];
          	
          	var ind = 0;
          	for(ind; ind<trCollection.length; ind++){
          	  var cell_id = $(trCollection[ind]).children('td').eq(colindx).attr("id");
          	  if(typeof cell_id != "undefined"){
          		cellIds.push(cell_id);
          	  }
          	}
          	var row = instance.countRows();
            var col = instance.countCols() + 1;
            
            var cmpCollection = getCurrentCellIdsComponents(cmptId, cellIds); // holds current selected index column contents before addition
            
            updateHtmlCollection(cmptId, cellIds, 'updateObject', row, col);
        	  
            this.alter("insert_col", selection.start.col);
            
            /* shift cell values column wise */
          	var tdLength = $(trCollection[0]).children('td').length;
          	var sCounter;
          	
          	/* loop for shifting cell values in last-secondLast column fashion */
          	for(sCounter=tdLength-1; sCounter > colindx ; sCounter--){
          		var newcellIds = [];
              	var newcellstoShift = [];
              	var ind = 0;
              	for(ind; ind<trCollection.length; ind++){              	  
              	  var cell_id = $(trCollection[ind]).children('td').eq(sCounter-1).attr("id"); //for last column          	  
              	  if(typeof cell_id != "undefined"){
              		newcellIds.push(cell_id);
              	  }
              	  
	          	  for(var cind=sCounter; cind > colindx; cind--){          		  
	          		  var cell_id2 = $(trCollection[ind]).children('td').eq(cind).attr("id"); //for second last column
	          		  if(typeof cell_id2 != "undefined"){
	          				newcellstoShift.push({"id": cell_id2, "row":ind, "col":sCounter});
	          				break;
	          		  }	          		  
	          	  }
              	}
              	//console.log("newcellIds "+newcellIds);
              	var newcmpCollection = [];
              	if((sCounter-1)==colindx){
              		newcmpCollection = cmpCollection; // for current selected index column contents before addition
              	}else{
              		newcmpCollection = getCurrentCellIdsComponents(cmptId, newcellIds);
              	}
              	if(newcmpCollection.length >0 && newcellstoShift.length > 0){
        ...
        'col_right': {
          ...
          callback: function(key, selection){
            ...
            /* shift cell values column wise */
            var tdLength = $(trCollection[0]).children('td').length - 1;
            var sCounter;
            
            /* loop for shifting cell values in secondLast-last column fashion */
            for(sCounter=tdLength-1; sCounter > colindx ; sCounter--){
          		var newcellIds = [];
              	var newcellstoShift = [];
              	var ind = 0;
              	for(ind; ind<trCollection.length; ind++){              	  
              	  var cell_id = $(trCollection[ind]).children('td').eq(sCounter).attr("id");              	  
              	  if(typeof cell_id != "undefined"){
              		newcellIds.push(cell_id);  //for second last column
              	  }              	  
              	  
              	  for(var cind=sCounter+1; cind > colindx; cind--){          		  
	          		  var cell_id2 = $(trCollection[ind]).children('td').eq(cind).attr("id");
	          		  if(typeof cell_id2 != "undefined"){
	          				newcellstoShift.push({"id": cell_id2, "row":ind, "col":sCounter+1}); //for last column
	          				break;
	          		  }	          		  
	          	  }
            ...
            var nextColCellIds = []; // to reset column cells next to current selected 
            var ind = 0;
          	for(ind; ind<trCollection.length; ind++){
          	   var cell_id = $(trCollection[ind]).children('td').eq(colindx+1).attr("id");
          	  
          	  if(typeof cell_id != "undefined"){
          		nextColCellIds.push(cell_id);
          	  }
          	}
          	
          	if(nextColCellIds.length > 0){
          	   updateHtmlCollection(cmptId, nextColCellIds); // reset column cells next to current selected
          	}
          	this.render(); // render table to show updates
        ...
        'remove_row': {
          ...
          callback: function(key, selection){
        	//check data in row
        	var isExistFormula = checkData(selection.start.row, 'remove_row', instance);
            var cmptId = $(instance.rootElement).attr("id").split("_")[1];
            /*check to avoid removal of all rows and cols */
            if(isAllCellsTriedToRemove($(instance.rootElement).attr("id"),selection)){
            	return false;
            }
            /* validate to avoid element deletion if present in branching */
            if(checkBranchingDuringTableAlter('remove_row',cmptId,selection)){
            	return false;
            }
          		 ...
          		 new Modal({id : 1,
		  				...
		  				containtText : "The field you're editing contains a formula. Are you sure you want to delete it? This cannot be undone.",
		  	    		 okActionEvent : function(){
		  	    			...
		  	    			cleanRowComponents(amount);
		  	    			that.alter("remove_row", selection.start.row, amount);
		  	    			shiftRowComponents();
				    	 },
				    	 cancelActionEvent : function(){
				    		 return false;
          	...
          	/* function to shift cell components row wise */
          	function shiftRowComponents(){
          	  var startRow = selection.start.row;
              var endRow = selection.end.row;
              var startCol = selection.start.col;          	  
            		    
              for(var rowindx = startRow; rowindx <= endRow; rowindx++){
               	var tdCollection = $(instance.getCell(rowindx, startCol)).parent().find("td");
               	/* remove existed formulas from current selected row */
               	if(isExistFormula){
               	  var ncellIds = [];
    	          var rind = 0;
    	          for(rind; rind<tdCollection.length; rind++){
    	            ncellIds.push($(tdCollection[rind]).attr("id"));
    	          }
    	          //console.log("ncellIds: "+ncellIds);
    	          for(var pk in ncellIds){	
    	            removeformulaFromCell(ncellIds[pk]);
    	          }
               	}
                ...
                /* loop for shifting cell values in second-first row fashion */
                for(sCounter=startRow; sCounter < trLength-1 ; sCounter++){
                	var newcellIds = [];
                   	var newcellstoShift = [];
                    	
                   	var nextTrParent1 = $(instance.rootElement).find("tr").eq(sCounter+2); //second row
                   	var nextTrParent2 = $(instance.rootElement).find("tr").eq(sCounter+1); //first row
                   	var ind = 0;
          	...
          	/* function to clean cell components of rows that will be removed */
          	function cleanRowComponents(amount){          	  
          	  var trCollection = $(instance.getCell(selection.start.row, selection.start.col )).parents("table").find("tr");
              trCollection.splice(0,1) //remove first unwanted tr that has th collection
                
              var trLength = trCollection.length;
              var startCol = selection.start.col;               
                
              if(trLength > 0){
            	  for(var aCnt=0; aCnt<amount;aCnt++){
      	        	  var remCellIds = [];
      	        	  var eachTdCollection = $(instance.getCell(trLength - 1 - aCnt, startCol)).parent().find("td");
      	        	  var ind = 0;
                  	  for(ind; ind<eachTdCollection.length; ind++){
                  		remCellIds.push($(eachTdCollection[ind]).attr("id"));                	    
                  	  }
                  	  //console.log("remCellIds: "+remCellIds);
                  	  if(remCellIds.length > 0){
                  		updateHtmlCollection(cmptId, remCellIds, 'hasFormulas'); // reset row cells along with formulas
                      }
        ...
        'remove_col': {
          ...
          var isExistFormula = checkData(selection.start.col, 'remove_col', instance);
          var cmptId = $(instance.rootElement).attr("id").split("_")[1];
          /*check to avoid removal of all rows and cols */
          if(isAllCellsTriedToRemove($(instance.rootElement).attr("id"),selection)){
          	return false;
          ...
          /* validate to avoid element deletion if present in branching */          
          if(checkBranchingDuringTableAlter('remove_col',cmptId,selection)){
          	return false;
          }
          ...
          var shiftedCellIds = [];
          	if(isExistFormula){
          		 ...
          		 new Modal({id : 1,
		  				...
		  				containtText : "The field you're editing contains a formula. Are you sure you want to delete it? This cannot be undone.",
		  	    		 okActionEvent : function(){
		  	    			...
		  	    			/* clean up components of columns to remove */
		  	          	    cleanColComponents(amount);		  	    			
		  	    			that.alter("remove_col", selection.start.col, amount);
		  	    			/* reset cell data according to updated html in each cell */
		  	    			if(shiftedCellIds.length > 0){
		  	            	  settableCellData(cmptId, shiftedCellIds);
		  	            	}
		  	    			shiftColComponents();
				    	 },
				    	 cancelActionEvent : function(){
          	  ...
          	  var prevColindx =  selection.start.col - 1; //get previous col index of current selected one
          	  
          	  if(prevColindx >= 0){
	           var trCollection = $(instance.getCell(selection.start.row, prevColindx)).parents("table").find("tr");
	           trCollection.splice(0,1) //remove first unwanted tr that has th collection
          	  }          		
          	  ...
          	  /* clean up components of columns to remove */
          	  cleanColComponents(amount);
          	  this.alter("remove_col", selection.start.row, amount);
          	  shiftColComponents();
	          ...
	          /*reset previous column */
	          if(prevColindx >= 0){
	           for(var cnt = prevColindx; cnt >=0; cnt--){	          	
	          	var ind = 0;
	          	for(ind; ind<trCollection.length; ind++){
	          	  var cell_id = $(trCollection[ind]).children('td').eq(cnt).attr("id");
	          	  if(typeof cell_id != "undefined"){
	          		  var divWrapper = $("#"+cell_id).find("div");
	          		  if(divWrapper.length == 0){	          			 
	          			instance.setDataAtCell(ind, cnt, "");
	          		  }else{
	          			 var txt = divWrapper.text();
	          			instance.setDataAtCell(ind, cnt, txt);
	          		  }
	          	  }	          		
	          	}	          	
	           }
          	...
          	/* function to shift cell components column wise */
          	function shiftColComponents(){
              var startRow = selection.start.row;
        	  var startCol = selection.start.col;
        	  var endCol = selection.end.col;
        		    
        	  for(var colindx = startCol; colindx <= endCol; colindx++){
            	var trCollection = $(instance.getCell(startRow, colindx)).parents("table").find("tr");
              	trCollection.splice(0,1) //remove first unwanted tr that has th collection
              	/* remove current selected column components  
              	var selectedCellIds = [];
              	var othrtInd = 0;
  	            for(othrtInd; othrtInd<trCollection.length; othrtInd++){
  	            	var selecedcell_id = $(trCollection[othrtInd]).children('td').eq(colindx).attr("id");
              	    if(typeof selecedcell_id != "undefined"){
              	    	selectedCellIds.push(selecedcell_id);
              	    }	
  	            }*/
  	            ...
  	            /* loop for shifting cell values in second-first column fashion */
  	            for(sCounter=startCol; sCounter < tdLength ; sCounter++){
  	          		var newcellIds = [];
  	              	var newcellstoShift = [];
  	              	var ind = 0;
  	              	for(ind; ind<trCollection.length; ind++){
  	              	  
  	              	/* remove existed formulas from current selected column */
  	                if(isExistFormula){
  	                  var ncell_id = $(trCollection[ind]).children('td').eq(sCounter).attr("id");              	  
    	              if(typeof ncell_id != "undefined"){
    	            	  removeformulaFromCell(ncell_id);
    	              }  	      	          
  	                }
  	              	
  	              	  var cell_id = $(trCollection[ind]).children('td').eq(sCounter+1).attr("id");              	  
  	              	  if(typeof cell_id != "undefined"){
  	              		newcellIds.push(cell_id); //second column
  	              	  }	              	  
  	              	  
  	              	  for(var cind=sCounter; cind <= tdLength; cind++){          		  
  		          		  var cell_id2 = $(trCollection[ind]).children('td').eq(cind).attr("id");
  		          		  if(typeof cell_id2 != "undefined"){
  		          				newcellstoShift.push({"id": cell_id2, "row":ind, "col":sCounter}); //first column
  		          				if($.inArray(cell_id2, shiftedCellIds) != -1){
  		          					shiftedCellIds.push(cell_id2);	
  		          				}
  		          				break;
  		          		  }	          		  
  		          	  }
  	            ...
  	            /*if(selectedCellIds.length > 0){
          		  updateHtmlCollection(cmptId, selectedCellIds, 'hasFormulas'); // reset column cells along with formulas
          	    }*/
        	  }
            }
          	
          	/* function to clean cell components of columns that will be removed */
          	function cleanColComponents(amount){          	  
  	          var trCollection = $(instance.getCell(selection.start.row, selection.start.col )).parents("table").find("tr");
              trCollection.splice(0,1) //remove first unwanted tr that has th collection
              
              var tdCollection = $(instance.getCell(selection.start.row, selection.start.col)).parent().find("td");
              var tdLength = tdCollection.length; 
              if(tdLength > 0){
            	  for(var aCnt=0; aCnt<amount;aCnt++){
      	        	  var remCellIds = [];
      	        	  var ind = 0;
                  	  for(ind; ind<trCollection.length; ind++){
      	        	    var remcell_id = $(trCollection[ind]).children('td').eq(tdLength - 1 - aCnt).attr("id");
                	    if(typeof remcell_id != "undefined"){
                	    	remCellIds.push(remcell_id);
                	    }
                  	  }
                  	  //console.log("remCellIds: "+remCellIds);
                  	  if(remCellIds.length > 0){
                  		updateHtmlCollection(cmptId, remCellIds, 'hasFormulas'); // reset column cells along with formulas
                  	  }
  ...
  ContextMenu.prototype.bindMouseEvents = function (){
    ...
    /*function contextMenuOpenListener(event){
      ...
      //if(event.target.nodeName != 'TD' && !(Handsontable.Dom.hasClass(event.target, 'current') && Handsontable.Dom.hasClass(event.target, 'wtBorder'))){
        if(event.target.nodeName != 'TD' && Handsontable.Dom.hasClass(event.target, 'wtBorder') && Handsontable.Dom.hasClass(event.target, 'wtSpreader')){
        return;
      }
      if(event.target.nodeName == 'TH' || Handsontable.Dom.hasClass(event.target, 'relative') || Handsontable.Dom.hasClass(event.target, 'manualColumnResizerHandle') ){
        return;
      }
...
*/
	  ...
	  function contextMenuOpenListener(event){
		    ...
		    /* custom code to enable/disable remove row or col menu items*/
		    if(this.instance.rootElement != undefined){
		    	var spreadTbl = $(this.instance.rootElement).find("table");
		    	if(spreadTbl.length>0){
		    		var trLen = spreadTbl.find("tr").length;
		    		var tdLen = spreadTbl.find("tr:eq(1)").find("td").length;		    		
		    		/*disable Remove Row menu */
		    		if(trLen == 2){	items.remove_row.disabled = true;}
		    		/*disable Remove Col menu */
		    		if(tdLen == 1){	items.remove_col.disabled = true;}
		    	}
		    }
			...
			if(isChrome){/*to adjust context menu position in chrome*/
				var menuTopVal = parseInt(menu.style.top, 10);
				menu.style.top = (menuTopVal - 70) + 'px';
			}
...
/*    var newCursor = cursorY;
    if(cursorY+230<600){
        newCursor = newCursor+160;
    }
    var cursor = {
      top:  newCursor,
 ...
 */
Handsontable.hooks.add('beforeGetCellMeta', function (row, col, cellProperties) {
...
(function(Handsontable){
  ...
  Handsontable.UndoRedo.ChangeAction = function (changes) {
    ...
    for( var i=0; i<changes[0].length; i++){
      if(changes[0][i] == null)
          changes[0][i] = "";
    }
  };
...
(function (Handsontable, CopyPaste, SheetClip) {
  ...
  function CopyPastePlugin(instance) {
    ...
    this.setCopyableText = function () {
      ...
      var bottomRight = 0;
      try{
    	  topLeft = selRange.getTopLeftCorner();
    	  bottomRight = selRange.getBottomRightCorner();
      }
      catch(e){
 ...
 * @constructor
 */
function MergeCells(mergeCellsSetting, instance) {
  this.instance = instance;
...
MergeCells.prototype.mergeOrUnmergeSelection = function (cellRange) {
	var topLeft = cellRange.getTopLeftCorner();
	  var bottomRight = cellRange.getBottomRightCorner();
  var info = this.mergedCellInfoCollection.getInfo(cellRange.from.row, cellRange.to.col);
  if (info) {
    //unmerge
	...
	/* custom code for merging formula cells */
	  
	var tblObj = arguments[1]; 
	var isExistFormula = checkRangeData(topLeft, bottomRight, this.instance,"merge");
...
var tmpData = this.instance.getData();
  ...
  if (info) {
    if (info.row === row && info.col === col) {
      ...
      //this.instance.
      //var changes = [];
     // instance.getData().setDataInputToArray(row,col,"");
      //clearMergeCellData(TD.id, instance);
     
      //changes.push([row, col, ""]);
      //this.instance.setDataAtCell(changes);
...
var init = function () {
  ...
  if (mergeCellsSetting) {
    if (!instance.mergeCells) {
      instance.mergeCells = new MergeCells(mergeCellsSetting, instance);
    }
  }
};
...
var addMergeActionsToContextMenu = function (defaultOptions) {
  ...
  defaultOptions.items.mergeCells = {
    ...
    callback: function () {
      ...
      //this.render();
       custom code for merging formula cells 
      this.mergeCells.mergeOrUnmergeSelection(this.getSelectedRange(), this);
      this.render();
    },
    disabled: function () {
      return !this.mergeCells.canMergeRange(this.getSelectedRange());
    }
  };
};*/
...
function WalkontableTable(instance, table) {
  ...
  if (!parent || parent.nodeType !== 1 || !this.wtDom.hasClass(parent, 'wtHolder')) {
    ...
    holder.style.display = 'table-cell';
...
WalkontableTable.prototype.refreshHiderDimensions = function () {
  ...
  if ((height !== Infinity || width !== Infinity) && !this.instance.getSetting('nativeScrollbars')) {
    ...
    var width = getTotalWidthTable(this); 
    this.hiderStyle.width = (width)+'px';
    this.hiderStyle.minWidth = "768px" //custom code for adjusting width"
  }
  else {
    ...
    if (rowHeaders.length) {
      ...
      for (var i = 0, ilen = rowHeaders.length; i < ilen; i++) {
        if (TH) {
          ...
          this.rowHeaderWidth += 75; //yes this is a cheat but it worked like that before, just taking assumption from CSS instead of measuring. TODO: proper fix
        }
      }
    }
    else {
...
function checkData(index, operation, instance){
	var totalRows = instance.countRows();
    var totalCols = instance.countCols();
	if(operation == 'remove_row'){
		for(var colIndex = 0; colIndex < totalCols; colIndex++){
			var cellId = 0;
			var cell = instance.getCell(index, colIndex);
			if(cell != null){
				cellId =cell.id;
			}
		...
		for(var rowIndex = 0; rowIndex < totalRows; rowIndex++){
			var cellId = 0;
			var cell = instance.getCell(rowIndex, index);
			if(cell != null){
				cellId = cell.id;
			}
			for(var i=0 ; i < question.formulas.length ; i++){
...
function checkRangeData(topLeft, bottomRight, instance, operation){
	var startRow = topLeft.row;
	var startCol = topLeft.col;
	var endRow = bottomRight.row;
	var endCol = bottomRight.col;
	var isFirstCell = false;
	var mergeFormulaCellFlag = false;
	for(var rowIndex = startRow; rowIndex <= endRow; rowIndex++){
      ...
      if(operation == "merge"){
        for(var i=0 ; i < question.formulas.length ; i++){
          if($.inArray(cellId, question.formulas[i].destinations) != -1){
            if(!isFirstCell){
            	mergeFormulaCellFlag = true;
          	}
          	else{
          		mergeFormulaCellFlag =  false;
          	}
          }
        }
      }
      else if(operation == "checkCellBelongToFormula"){
        var componentId = $(instance.rootElement).attr("id").split("_")[1];
        for(c in question.activePage.components){
          var comp = question.activePage.components[c];
          if($.isFunction(comp.getFullComponentId)){
            if(comp.getFullComponentId() == componentId && comp.type == "table"){
              for(var rowIndex = startRow; rowIndex <= endRow; rowIndex++){
                for(var colIndex = startCol; colIndex <= endCol; colIndex++){
                  var cellId = 0;
                  var cell = instance.getCell(rowIndex, colIndex);
                  ...
                  var component = comp.getItemValById(comp.cellComponentCollection, cellId, "component");
                  if(component != ""){
                    if(component.type == "Input field"){
                      ...
                      if(contFor == "number" || contFor == "percentage" || contFor == "currency"){
                         for(var i=0 ; i < question.formulas.length ; i++){
                            ...
                            if($.inArray(cellId, formula.getStudentEntryIds(question.formulas[i].equation)) != -1){
                                return true;
                            }
                          }
                      } else{
...
function showCautionIcon(studentEntryId, source){
	...
	if(questionFormulas.indexOf(studentEntryId) != -1){
		...
		for(var i = 0 ; i < question.formulas.length ; i++){
			...
			if(formulaString.indexOf(studentEntryId) != -1){
				...
				question.refreshFormulaDestinations(question.formulas[i]);
			}
		}			 
...
function deleteComponent(topLeft, bottomRight, instance, isForumla){
	var componentId = $(instance.rootElement).attr("id").split("_")[1];
	var startRow = topLeft.row;
	var startCol = topLeft.col;
	var endRow = bottomRight.row;
	    	...
	    	if(comp.getFullComponentId() == componentId && comp.type == "table"){
	    		for(var rowIndex = startRow; rowIndex <= endRow; rowIndex++){
	    			for(var colIndex = startCol; colIndex <= endCol; colIndex++){
	    				...
	    				/* validate to avoid element deletion if present in branching */
	    				if(comp.isBranched){
	    				  var onlyCellId = Handsontable.helper.spreadsheetColumnLabel(colIndex) + rowIndex;
	    				  var isCellBranched = checkForBranching(comp,componentId+onlyCellId);
	    				  if(isCellBranched){
	    					return false;
	    				  }
	    				}
	    				...
	    				var component = comp.getItemValById(comp.cellComponentCollection, cellId, "component");
	    				if(component != ""){
	    					comp.changeValById(comp.cellHTMLCollection, cellId, "itsHtml", '');
	    					comp.changeValById(comp.cellComponentCollection, cellId, "component", '');
	    					...
	    					if(component.type != "Label"){
				              ...
				              if(component.contentFormat){
				                component.contentFormat.type = "";
				                // $("#CellComponentSpecificProperties").html();
				              }
				              if(component.type){
				                component.type="";
				              }
	    				}
	    				comp.itsTableInstance.selectCell(rowIndex,colIndex);
	    				comp.populateEditorProperties(cellId);
	    			}
	    		}
	    	}
		}
...
function updateHtmlCollection(componentId, cellId, callBackEvent, row,col){
	var c=0;
	for(c in question.activePage.components){
		var comp = question.activePage.components[c];
		if(comp.type == "table" && comp.getFullComponentId() == componentId){
			var kk =0;
      if(callBackEvent == "updateObject"){
        comp.createSpreadsheetData(row, col);
      }
			for(kk in cellId){
				//comp.changeValById(comp.cellHTMLCollection, cellId[kk], "itsHtml", "");
				/* get cell html */
				try{
	   		   		 var cellObj = comp.cellHTMLCollection.filter(function(e){
    	        	  		return e.id == cellId[kk];
    	        	  	});
	            }catch(err){
	            	cellObj = {};
	            }
	            ...
	            /* get cell component */
	            try{
	   		   		 var cellCompObj = comp.cellComponentCollection.filter(function(e){
   	        	  		return e.id == cellId[kk];
   	        	  	});
	            }catch(err){
	            	cellCompObj = {};
	            ...
	            if(cellObj.length > 0){
	            	var m=0;
	            	for(m in cellObj){
	            		if(cellObj[m].itsHtml != ""){
	            			cellObj[m].itsHtml = "";
	            		}
	            	}
	            }
	            /* remove cell components */
	            if(cellCompObj.length > 0){
	            	var m=0;
	            	for(m in cellCompObj){
	            		if(cellCompObj[m].component != ""){
	            			cellCompObj[m].component = "";
	            		}
	            	}
	            }
	            
	            /* remove cell formulas */
	            if(callBackEvent == "hasFormulas"){
				  removeformulaFromCell(cellId[kk]);
				}
			}			
		}
	}
...
function shiftHtmlCollection(componentId, shiftCells, cmpCollection){
	var c=0;
	for(c in question.activePage.components){
		var comp = question.activePage.components[c];
		if(comp.type == "table" && comp.getFullComponentId() == componentId){
			var kk =0;      
			for(kk in shiftCells){
				//comp.changeValById(comp.cellHTMLCollection, cellId[kk], "itsHtml", "");
				try{
	   		   		 var cellObj = comp.cellHTMLCollection.filter(function(e){
    	        	  		return e.id == shiftCells[kk].id;
    	        	  	});
	            }catch(err){
	            	cellObj = {};
	            }
	            ...
	            if(cellObj.length > 0){
	            	...
	            	for(m in cellObj){
	            		/*if(cellObj[m].itsHtml == ""){
	            			cellObj[m].itsHtml = shiftCells[kk].itsHtml;
	            		}*/
	            		if(cellObj[m].id == shiftCells[kk].id){
	            			//cellObj[m].itsHtml = shiftCells[kk].itsHtml;
	            			var citem = cmpCollection[kk];
	            			var compElement =  citem.component;
	            			var newCellId = shiftCells[kk].id;
	            			if(compElement != "" && typeof compElement != "undefined"){
	            				comp.itsTableInstance.deselectCell();
	            				comp.itsTableInstance.selectCell(shiftCells[kk].row,shiftCells[kk].col);
	            				
	            				//get selected row cols
	            				var cellSelection = comp.itsTableInstance.getSelected();
	            				//set selected cell id
	                            var itscellId = Handsontable.helper.spreadsheetColumnLabel(cellSelection[1]) + cellSelection[0];
	                            if(typeof itscellId != "undefined" && itscellId != ""){
	                              	$("span#CellRepresentId_"+componentId).text(itscellId);
	                            }
	            				
	            				var newCellCmp = null;
	            				switch(compElement.type){
	            				 case "Input field":
	            					 $("#inputField"+componentId).trigger("click");
	            					 newCellCmp = comp.getItemValById(comp.cellComponentCollection, newCellId, "component");
	            					 newCellCmp.graded = compElement.graded;
	            					 newCellCmp.acceptAnyAnswer = compElement.acceptAnyAnswer;
	            					 newCellCmp.answer.text = compElement.answer.text;
	            					 var conentFormatter=compElement.contentFormat;
	            				 ...
	            				 break;
	            				 case "Checkbox":
	            					 $("#CheckBox"+componentId).trigger("click");
	            					 newCellCmp = comp.getItemValById(comp.cellComponentCollection, newCellId, "component");
	            					 
	            					 if(newCellCmp.type != compElement.type){
	            						 comp.addCellComponent('CheckBox');
	            						 newCellCmp = comp.getItemValById(comp.cellComponentCollection, newCellId, "component");
	            					 }
	            					 
	            					 newCellCmp.graded = compElement.graded;
	            					 newCellCmp.acceptAnyAnswer = compElement.acceptAnyAnswer;
	            					 newCellCmp.options[0].right = compElement.options[0].right;
	            					 newCellCmp.options[0].editor.text = compElement.options[0].editor.text;
	            					 $("#checkboxValSave_"+newCellId).trigger("click");
		            			 break;
	            				 case "Dropdown menu":
	            					 $("#dropDown"+componentId).trigger("click");
	            					 newCellCmp = comp.getItemValById(comp.cellComponentCollection, newCellId, "component");
	            					 
	            					 if(newCellCmp.type != compElement.type){
	            						 comp.addCellComponent('dropDown');
	            						 newCellCmp = comp.getItemValById(comp.cellComponentCollection, newCellId, "component");
	            					 }
	            					 
	            					 newCellCmp.graded = compElement.graded;
	            					 newCellCmp.acceptAnyAnswer = compElement.acceptAnyAnswer;
	            					 newCellCmp.sections[0].editor.text = compElement.sections[0].editor.text;
	            	    				$("#" + newCellCmp.sections[0].editor.id).html($("#"+compElement.sections[0].editor.id).html());
	            	    				var i=0;
	            	    				for (i in compElement.sections[0].options) {
	            	    					var oldSection = compElement.sections[0];
	            	    				...
	            	    				$("#dropDownSave_"+newCellId).trigger("click");
	            					 
		            			 break;
	            				 case "Label":
	            					 $("#label"+componentId).trigger("click");
	            					 newCellCmp = comp.getItemValById(comp.cellComponentCollection, newCellId, "component");
	            					 if(newCellCmp.type != "Label" || newCellCmp == ""){
	            						$("#label"+componentId).trigger("click");
		            					newCellCmp = comp.getItemValById(comp.cellComponentCollection, newCellId, "component");
	            	    			 }
	            					 newCellCmp.graded = compElement.graded;
	            					 if(newCellCmp.editor!= undefined){
	            						 newCellCmp.editor.text = compElement.editor.text;
	            					 }
	            					 
	            					 
	            					 /* reset cell data  */
	            					 var rowNum = $("#"+newCellId).parent().index();
	            					 var colNum = $("#"+newCellId).index() - 1;
	            					 //comp.itsTableInstance.setDataAtCell(rowNum, colNum, "");
	            					 
	            					 /* get cell html */
	            					 //var cellHtml = comp.getItemValById(comp.cellHTMLCollection, citem.id, "itsHtml");
	            					 var cellHtml = comp.getItemValById(comp.cellHTMLCollection, newCellId, "itsHtml");
	            					 //if(cellHtml == ""){
	            					   cellHtml = compElement.editor.text;
	            					 //}
	            					 comp.itsTableInstance.setDataAtCell(rowNum, colNum, cellHtml);
	            					 comp.changeValById(comp.cellHTMLCollection, citem.id, "itsHtml", "");
	            					 comp.changeValById(comp.cellHTMLCollection, newCellId, "itsHtml", cellHtml);
	            					 $("#"+newCellId).html(cellHtml);
	            					 /* add new formulas during shift */
	            					 for(var i=0 ; i < question.formulas.length; i++){
	            	                  	if($.inArray(citem.id, question.formulas[i].destinations) != -1){
	            	                  		var cellHTmlText = $(cellHtml).text();
	            	                  		comp.itsTableInstance.setDataAtCell(rowNum, colNum, cellHTmlText);
	            	                  		question.formulas[i].addDestination(newCellId);
	            	                  		newCellCmp.ranges = [];
	            	                  		var ranges = [];
	            	                  		var oldRanges = compElement.ranges;
	            	                  		var newId = newCellCmp.id;
	            	                  		var itsCellIndex = newCellId.replace(componentId,""); // extract only selected cell row-col combination
	            	                  		if(oldRanges != null){
	            	                  			for(var index = 0; index < oldRanges.length; index++){
	            	                  			  var rangeObj = oldRanges[index];
	            	                  	          var minEConfig = {
	            	                  		           id : rangeObj.componentIdPrefix + newId+itsCellIndex+rangeObj.idPrefix
	            	                  		      ...
	            	                  		      var rangeConfig = {
	            	                  		           id : rangeObj.id,
	            	                  		           componentId : newId+itsCellIndex,
	            	                  		           componentIdPrefix : comp.pageIdPrefix+comp.pageId+comp.idPrefix,
	            	                  		       };
	            					 ...
	            					 /* remove formula related */	            					 
	            					 removeformulaFromCell(citem.id);
			            		 break;
			            		 default:
	            				}
	            			}
	...
	console.log("comp.itsTableInstance.getdata : "+comp.itsTableInstance.getData());
}
/*function clearMergeCellData(cellId, instance){
	var componentId = $(instance.rootElement).attr("id").split("_")[1];
	var component = null;
	for(c in question.activePage.components){
		var comp = question.activePage.components[c];
	...
	component.changeValById(component.cellHTMLCollection, cellId, "itsHtml", '');
	component.changeValById(component.cellComponentCollection, cellId, "component", '');
	question.resetGradeCount();
}*/
function getCurrentCellIdsComponents(componentId, cellIds){
	var compArr = [];	
	var c=0;
	for(c in question.activePage.components){
		var comp = question.activePage.components[c];
		if(comp.type == "table" && comp.getFullComponentId() == componentId){
			var kk =0;
			for(kk in cellIds){
				var citem = comp.getItemValById(comp.cellComponentCollection, cellIds[kk], "component");
				var row = comp.getItemValById(comp.cellComponentCollection, cellIds[kk], "row");;
				var col = comp.getItemValById(comp.cellComponentCollection, cellIds[kk], "col");
				compArr.push({"id":cellIds[kk], "row":row, "col":col, "component":citem});
			}
		}
	}
...
function removeformulaFromCell(itsCellId){
	for(var i=0 ; i < question.formulas.length ; i++){
		var formula = question.formulas[i];
        if($.inArray(itsCellId, formula.destinations) != -1){
        	formula.removeDestination(itsCellId);
        }
        formula.equation=formula.equation.replace("&#160;","");
		studentEntries = formula.getStudentEntryIds(formula.equation);	            				          		
    }
}
/* to set table cell data */
function settableCellData(componentId, cellIds){
	var compArr = [];	
	var c=0;
	for(c in question.activePage.components){
		var comp = question.activePage.components[c];
		if(comp.type == "table" && comp.getFullComponentId() == componentId){
			...
			for(kk in cellIds){
			    var tdId = cellIds[kk];
			    var tdHtml = comp.getItemValById(comp.cellHTMLCollection, tdId, "itsHtml");
			    var tdText = "";
			    if(hasHtmltags(tdHtml)){
			    	tdText = $(tdHtml).text();
			    }else{
			    	tdText = tdHtml;
			    }			    
			    var rownum = $("#"+tdId).parent().index();;
			    var colnum = $("#"+tdId).index() - 1;
			    comp.itsTableInstance.setDataAtCell(rownum, colnum, tdText);
			}
		}
	}
...
/* to check if string contain html tags */
function hasHtmltags(htmlString){
  if (htmlString.match(/<(\w+)((?:\s+\w+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)>/)) {
	 return true;
  }
  return false;
}

/* to reset related html in table cells */
function resetCellHtml(componentId){
	var c=0;
	for(c in question.activePage.components){
		var comp = question.activePage.components[c];
		if(comp.type == "table" && comp.getFullComponentId() == componentId){
			/*for(var i=0 ; i < question.formulas.length ; i++){
				var formula = question.formulas[i];
		        var fmkey = 0;
		        for(fmkey in formula.destinations){
		        	var fCellId = formula.destinations[fmkey];
		        	var fCellHtml = comp.getItemValById(comp.cellHTMLCollection, fCellId, "itsHtml");
		        	if(fCellHtml != ""){
		        		$("#"+fCellId).html(fCellHtml);
		        		//$("#"+fCellId).data("change",true);
		        	}
		        }
		    }*/
			for(var i=0 ; i < comp.cellHTMLCollection.length ; i++){
				var cellElem = comp.cellHTMLCollection[i];
				var cId = cellElem.id; 
				var cHtml = cellElem.itsHtml;
				if(cHtml != ""){
	        		$("#"+cId).html(cHtml);
	        		$("#"+cId).data("change",true);
	        	}
			}
		}
	}
...
/* to update spreadsheetData on table insert row/col operations */
function updateSpreadSheetData(componentId, row, col){
  var c=0;
  for(c in question.activePage.components){
	var comp = question.activePage.components[c];
	if(comp.type == "table" && comp.getFullComponentId() == componentId){
...
function getTotalWidthTable(instance){
	 var componentId =$(instance.TABLE).parents(".pageSection").attr("id")
	 var tableWidth= 0;
	 var comp = null;
	 for(c in question.activePage.components){
		comp = question.activePage.components[c];
		if(comp.type == "table" && comp.getFullComponentId() == componentId){
		  var colwidths = comp.colWidth;
		}
	 }
	  var i = 0;
	  var count = instance.columnFilter.total;
      for(i = 0; i < count; i++){
      ...
      if(tableWidth < 768){
      	$("#"+componentId).css("width","800px");
      }else{
          $("#"+componentId).css("width",(tableWidth + 50)+"px");
      }
      return tableWidth;
}
/*for remove_row/col operation */
function checkBranchingDuringTableAlter(operation,componentId,selection){
	var startRow = 0;
	var startCol = 0;
	var endRow = 0;
	var endCol = 0;
	...
	var c = 0;
	for(c in question.activePage.components){
		component = question.activePage.components[c];
		if(component.type == "table" && component.getFullComponentId() == componentId){
		  break;
		}
	 }
	switch(operation){
	  case 'remove_row':
		  endCol = component.colCount - 1;
		  startCol = 0;
	  break;
	  case 'remove_col':
		  endRow = component.rowCount - 1;
		  startRow = 0
	  break;	  
	  default:
	}
	
	if(component.isBranched){		
		for(var rowIndex = startRow; rowIndex <= endRow; rowIndex++){
			for(var colIndex = startCol; colIndex <= endCol; colIndex++){
				var onlyCellId = Handsontable.helper.spreadsheetColumnLabel(colIndex) + rowIndex;
				if(checkForBranching(component,componentId+onlyCellId)){
					return true;
				}
			}
		}		  
...
/*function to check whether tablke component is present in branching or not */
function checkForBranching(component,cellId){	
	var hasBranchOnCell = false;
  	var isFormulaDest = false;
	var isInputField = false;
  	var cellComp = component.getItemValById(component.cellComponentCollection, cellId, "component");
	    ...
	    }else if(cellComp.type == 'Input field'){
	    ...
	    if(isFormulaDest || isInputField){
			   ...
			   hasBranchOnCell = true;
		   }
  	
  	 return hasBranchOnCell;
}
/*to validate if all cells tried to remove */
function isAllCellsTriedToRemove(cmptId,selection){
	var isAllTired = false;
	var startRow = selection.start.row;
	var startCol = selection.start.col;
	var endRow = selection.end.row;
	var endCol = selection.end.col;
	...
	if(startRow != endRow || startCol != endCol){
		var trLength = $("#"+cmptId).find("tr").length - 2; /*-1 for th's tr and -1 for 0 indexed rows in handsontable */
		var tdLength = $("#"+cmptId).find("tr:eq(1)").find("td").length -1; /* -1 for 0 indexed cols in handsontable */
		
		if(startRow==0 && startCol==0 && endRow == trLength && endCol == tdLength){
			isAllTired = true;
			new Modal({
				id : 1,
				headingText : "Warning ! ",
				containtText : "All rows/columns cannot be Deleted.",
			}).getWarningModal();
		}
	}
