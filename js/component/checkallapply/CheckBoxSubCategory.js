 ...
 * ========================================================================
 * Purpose of this object to have all CheckBox Sub-Category specific properties like 
 * subCategoryIdPrefix,minPoint, maxPoint, textIdPrefix, editor
 * ======================================================================== */
var CheckBoxSubCategory = function(options){ 
	this.id=null;
	...
	this._addSubCategory = CheckBoxSubCategory.BindAsEventListener( this, this.addSubCategory );	
	this._removeSubCategory = CheckBoxSubCategory.BindAsEventListener( this, this.removeSubCategory );
	this._update = CheckBoxSubCategory.BindAsEventListener( this, this.update );
	this._populateSubCategoryProperties = CheckBoxSubCategory.BindAsEventListener( this, this.populateSubCategoryProperties );
	this._setAwardPoint = CheckBoxSubCategory.BindAsEventListener( this, this.setAwardPoint );
	this._filter = CheckBoxSubCategory.BindAsEventListener( this, this.filter );
	this._notApplicableSubcategory = CheckBoxSubCategory.BindAsEventListener(this, this.notApplicableSubcategory);
};

//add event helper method on element id
CheckBoxSubCategory.addEvent = function(oTarget, sEventType, fnHandler,args) {
	$("#"+oTarget).unbind(sEventType);
	$("#"+oTarget).bind(sEventType,args,fnHandler);
};
//add event helper method on element class
CheckBoxSubCategory.addEventOnClass = function(oTarget, sEventType, fnHandler,args) {
	$("."+oTarget).unbind(sEventType);
	$("."+oTarget).bind(sEventType,args,fnHandler);
};
//use call to borrow context
CheckBoxSubCategory.BindAsEventListener = function(object, fun) {
...
CheckBoxSubCategory.prototype = { 
		 ...
		 addSubCategory:function(){
			 var i=0;
	       	  for(i in question.activePage.components){
	       		...
	       		var newSubCategory = new CheckBoxSubCategory(config);
       ...
       deleteSubCategory:function(removeRange){
        ...
        layout:function(isLast){
        	var hidden =isLast?'':'hidden';
        	 var htmlelement="";
        	 htmlelement+='<div class="CheckBoxSubCategoryDiv">';  
        ...
        editorLayout:function(){
        	var dataDivAttr = this.editor.text !='' && this.editor.text !='&nbsp;'? 'data-div-placeholder-content' : '';
        	var htmlelement="";
       ...
       afterLayout:function(){
    	   CheckBoxSubCategory.addEvent( "plus_"+this.editor.id, "click", this._addSubCategory );
    	   CheckBoxSubCategory.addEvent( "minus_"+this.editor.id, "click", this._removeSubCategory );
    	   CheckBoxSubCategory.addEvent( this.editor.id, "blur", this._update );
    	   CheckBoxSubCategory.addEvent( this.editor.id, "click", this._populateSubCategoryProperties);
    	   CheckBoxSubCategory.addEvent( this.editor.id, "paste", this._filter );
      ...
      afterSubCategoryPropertyLayout:function(){
    	  CheckBoxSubCategory.addEvent( "notApplicableOption"+this.editor.id, "click", this._notApplicableSubcategory );
       	
    	  CheckBoxSubCategory.addEvent("optionsRadios1"+this.editor.id, "click", this._setAwardPoint);
    	  CheckBoxSubCategory.addEvent("optionsRadios2"+this.editor.id, "click", this._setAwardPoint);
